...
export const useTruckAnimation = ({ runTrigger, onArrive, nodes }: UseTruckAnimationParams) => {
  ...
  useFrame((_, delta) => {
    ...
    if (innerGroupRef.current) {
      ...
      const targetPitch = currentPos.y > -0.2 && currentPos.y < 3.7 ? rampPitch : basePitch;
